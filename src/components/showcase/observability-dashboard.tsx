    ...
    <div className="w-full max-w-sm mx-auto">
      ...
      {hoveredId && (
        <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="mt-3 rounded-lg border border-white/10 bg-white/[0.03] p-3">
          <p className="text-xs text-zinc-300">{nodes.find((n) => n.id === hoveredId)?.description}</p>
        </motion.div>
      )}

      {!hoveredId && (
        <p className="mt-3 text-center text-xs text-zinc-600">Hover over the diagram to explore each component.</p>
      )}
    </div>
  );
}
