        ...
        {(["catalog", "scaffolder"] as const).map((tab) => (
          <button
            ...
            className={`flex-1 rounded-md px-3 py-2 text-xs font-medium transition-all ${
              activeTab === tab ? "bg-accent/15 text-accent" : "text-muted hover:text-foreground-secondary"
            }`}
          >
      ...
      <AnimatePresence mode="wait">
        {activeTab === "catalog" ? (
          <motion.div key="catalog" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.2 }} className="space-y-2">
            <div className="flex items-center gap-3 text-xs text-foreground-tertiary px-1">
            ...
            <div className="space-y-1.5">
              {services.map((svc) => (
                <div key={svc.name} className="flex items-center justify-between rounded-lg border border-border bg-surface px-4 py-3 transition-colors hover:border-border-strong">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-[11px] text-muted">{svc.owner}</span>
                      </div>
                    </div>
                  </div>
          ...
          <motion.div key="scaffolder" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.2 }} className="space-y-3">
            <p className="text-xs text-foreground-tertiary px-1">Choose a template to scaffold a new service — infrastructure, CI/CD, and docs generated automatically.</p>
            {[
              ...
              <div key={tpl.name} className="flex items-center justify-between rounded-lg border border-border bg-surface px-4 py-3 transition-colors hover:border-accent/30 cursor-pointer group">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground group-hover:text-accent transition-colors">{tpl.name}</p>
                  <p className="text-[11px] text-muted mt-0.5">{tpl.desc}</p>
                </div>
                <Rocket className="h-4 w-4 shrink-0 text-muted group-hover:text-accent transition-colors" />
              </div>
            ))}
          </motion.div>
        )}
