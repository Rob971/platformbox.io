...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      {
        // Exclude public onboarding aliases as well as /admin and /admin/* — proxy.ts (G0) owns the /admin header
        ...
        source: "/((?!admin|assessment/(?:start|complete|cancelled|activate|resend-activation)(?:/|$)).*)",
        headers: [
          {
            key: "X-Frame-Options",
            value: "DENY",
