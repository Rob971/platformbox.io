...
Note: `src/lib/capability-claims.json` is generated by
`scripts/generate-capability-claims.mjs` (`npm run generate-claims`), joining the
IDP capability-registry (reference status) with the delivery capability
catalogue (`config/capabilities/platformbox-capabilities.json` in
platformbox-delivery — delivery status, packages, website_claim_allowed). It
deliberately excludes `proven_with`, which holds engagement ids and is therefore
customer information. Regenerate it after either source changes: a projection
that drifts silently is a credibility problem, not a build problem.
